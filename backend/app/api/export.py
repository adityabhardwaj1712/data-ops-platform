...
from app.processing.exporter import exporter
...
async def export_dataset(
    ...
    try:
        ...
        # Export the data
        if request.is_client_package:
            # Prepare metadata summary for the package
            # This is a bit simplified, ideally would calculate actual stats from DatasetVersion
            meta_summary = {
                "confidence_summary": {"score": dataset_version.change_summary.get("avg_confidence", 85) if dataset_version.change_summary else 90},
                "is_human": any(t.type == "HUMAN" for t in job.tasks) if hasattr(job, 'tasks') else False
            }
            # Get artifact paths if available
            artifact_paths = dataset_version.confidence_summary.get("artifact_paths", []) if dataset_version.confidence_summary else []
            
            export_response = await exporter.create_client_package(
                data=data,
                request=request,
