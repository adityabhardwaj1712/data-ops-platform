...
class WorkerService:
    ...
    def __init__(self, concurrency: int = 2, poll_interval: int = 5):
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    await self._update_job_status(job_id, JobStatus.COMPLETED, result)
        ...
        try:
            async for db in get_db():
                ...
                if job:
                    job.status = status
                    if result:
                        # Store result in job config for now
                        if not job.config:
                            job.config = {}
